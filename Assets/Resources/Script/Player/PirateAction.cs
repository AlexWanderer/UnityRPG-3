...
public class PirateAction : PlayerAction {
    ...
    public void Set_TouchSkill()
    {
        ...
        Invoke("Stop_TouchSkillEffect", Init_Infomation.Touch_time);

        // Player의 HpBar옆에 도발아이콘을 띄우게 한다.
        UIManager.Get_Inctance().Set_PlayerState(transform.parent.name, "Provocation", Init_Infomation.Touch_time);
        // 모든 몬스터들의 Target을 자기로 고정시킨다. TouchSkill_Time만큼 지나면 Target은 다시 재조정된다.
        MonsterManager.Get_Inctance().Set_ParticularTarget(gameObject, Init_Infomation.Touch_time);
    ...
    public override void Special_Skill()
    {
        ...
        {
            SpecialSkill_NPC[i].GetComponent<Henchman_Action>().Attack = Init_Infomation.Special_value;
            SpecialSkill_NPC[i].GetComponent<Henchman_Action>().Time_duration = Init_Infomation.Special_time;
        }
