...
public class PlayerManager : MonoBehaviour
{
   public enum STATE
    ...
    public void Ready_State(RecvCharaterInfo[] SelectCharaterNames)
    {
        for (int index = 0; index < SelectCharaterNames.Length; index++)
        {
            if(SelectCharaterNames[index] == null) { continue; }

            string path = string.Format("Prefabs/Charater/{0}", SelectCharaterNames[index].Name);

            GameObject CharaterPrefab = Resources.Load(path) as GameObject;

            if (CharaterPrefab == null)
            {
                Debug.Log("CharaterPrefab Load Faild!!");
                return;
            }

            GameObject Charater = Instantiate(CharaterPrefab);
            Characters[index] = Charater;
            Charater.transform.parent = transform.GetChild(index);
            Charater.transform.localPosition = Vector3.zero;
            Charater.name = SelectCharaterNames[index].Name;
            Charater.GetComponent<PlayerAction>().Set_Info(SelectCharaterNames[index]);
    ...
    public GameObject Set_PlayerHeal(float heal_value)
    {
        ...
        if(target.Hp > target.Init_Infomation.Hp)
        {
            target.Hp = target.Init_Infomation.Hp;
        }
    ...
    public GameObject Get_Player_LowHp()
    {
        ...
        for(int i = 0; i < Characters.Length; i++)
        {
            ...
            PlayerAction player = Characters[i].GetComponent<PlayerAction>();
            float value = player.Hp / player.Init_Infomation.Hp;
